...
interface LeftMindmapThumbnailProps {
  structure: MindmapNode;
  className?: string;
}

export function LeftMindmapThumbnail({ structure, className }: LeftMindmapThumbnailProps) {
  const rootChildren = structure.children || [];

  const branches = rootChildren.map((child, i) => buildBranch(child, 1, i, -1, `L${i}`));
  ...
  const totalH = branches.reduce((s, b) => s + subtreeHeight(b), 0)
    + Math.max(0, branches.length - 1) * THUMB_SIBLING_GAP;
  const maxH = Math.max(totalH, THUMB_NODE_H);

  root.y = maxH / 2 - root.height / 2;
  root.x = 0;

  let leftY = maxH / 2 - totalH / 2;
  for (const branch of branches) {
    positionLeft(branch, root.x - branch.width - THUMB_LEVEL_GAP, leftY);
    leftY += subtreeHeight(branch) + THUMB_SIBLING_GAP;
  }

  const allNodes = [root, ...branches];
