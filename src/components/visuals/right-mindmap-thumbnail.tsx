...
interface RightMindmapThumbnailProps {
  structure: MindmapNode;
  className?: string;
}

export function RightMindmapThumbnail({ structure, className }: RightMindmapThumbnailProps) {
  const rootChildren = structure.children || [];

  const branches = rootChildren.map((child, i) => buildBranch(child, 1, i, 1, `R${i}`));
